...
`session-log.md`·`decision-log.md`·`프롬프트-로그.md`는 append-only라 매 세션 계속 길어진다. 300줄을 넘으면 오래된 항목을 `_아카이브/<로그명>-<기간>.md`로 이월하는 로테이션이 적용된다(AGENTS §4, 2026-07-05 최초 시행 — 항목 삭제 없이 이월만, 메인은 최신 활성 구간만 유지). `_아카이브/` 파일은 이 스킬의 `TARGETS` 대상이 아니다(자동 TOC 없음, 필요하면 원본 순서 그대로 읽는다). 새 세션이나 팀원이 특정 날짜·주제를 찾으려면 지금까지는 파일 **전체**를 읽어야 했다 — AI든 사람이든 마찬가지다.
