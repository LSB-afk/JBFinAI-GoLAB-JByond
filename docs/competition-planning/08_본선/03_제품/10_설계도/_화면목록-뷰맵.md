...
| 뷰 키            | 판정     | 매핑     | 비고                                                                                                                        |
| -------------- | ------ | ------ | ------------------------------------------------------------------------------------------------------------------------- |
| `rm-dashboard` | [부분특화] | 없음(부분) | 레거시 단일 페이지(`app.js:1769 rmDashboardPage`) — **0226bd6부터 신규 rm-officer 하네스(16뷰)와 병존**, 레거시는 재유입 가드로 격리. `CaseQueueItem` 골격 재사용 가능하나 RM 전용 KPI 조합·집계 로직은 미정 `[추정]` |
