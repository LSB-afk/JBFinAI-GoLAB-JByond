...
정적 앱이라 별도 번들러는 사용하지 않지만, npm 스크립트로 실행·검증 명령을 통일했습니다.
...
`build`와 `test`는 정적 MVP의 필수 파일, 문서 키워드, JS 문법을 확인하는 `scripts/verify_static.py`를 실행합니다. `test:e2e`는 Playwright로 핵심 시나리오와 반응형 화면을 검증하고 `/test-results/screenshots/`에 캡처를 저장합니다.
