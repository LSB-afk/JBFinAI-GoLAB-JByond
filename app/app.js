...
const navigation = [
  ...
  },
  {
    section: "고객지원 업무",
    items: [
      { id: "jeonse", icon: "shield", label: "전세 안심 점검", description: "확장 로드맵", countKey: "jeonse" },
...
/* 실동작 에이전트(4~5개)와 전시용 확장 조직도(14개)를 명확히 분리한다.
   실동작: 조율(오케스트레이터), 여신 분류, 보이스피싱 차단, 문서/체크리스트, 감사·승인 통제. */
const liveAgentIds = new Set(["orchestrator", "cashflow", "fraud", "contract-check", "compliance"]);

function agentExecutionBadge(agentOrId) {
  const id = typeof agentOrId === "string" ? agentOrId : agentOrId && agentOrId.id;
  return liveAgentIds.has(id)
    ? '<span class="status-pill status-approved exec-badge">실동작</span>'
    : '<span class="status-pill status-new exec-badge">확장 예정</span>';
}
...
function renderMetrics() {
  ...
  const cards = [
    ["담당자 승인 대기", `${data.pending.length}건`, "고객 대상 행동 전 RM/준법 검토 필요 — 여신 Hero 흐름"],
    ["고객 영향 조치 보류", `${data.blocked.length}건`, "보이스피싱·준법 리스크로 담당자 확인 전 발송 보류"],
    ["근거 연결률", `${data.evidenceRate}%`, "검토 근거가 연결된 관리 건 비율"],
    ["전세 안심 점검 · 로드맵", `${data.jeonseRisk.filter((item) => item.riskScore >= 85).length}건`, "확장 로드맵 후보 — Hero 실동작 범위 아님"],
  ];
...
function heroDemoLaunchView() {
  return `
    <section class="workspace-panel hero-demo-panel" aria-label="Hero 데모 시작">
      <p class="eyebrow">Hero 데모 · 실동작 2개 도메인 (여신 · 보이스피싱)</p>
      <div class="hero-demo-actions">
        <a class="primary-button" href="?demo=sme">GP-1 여신(소상공인·SME) 데모 시작</a>
        <a class="primary-button" href="?demo=phishing">GP-2 보이스피싱 차단 데모 시작</a>
        <a class="secondary-button" href="?demo=jeonse">전세 보호 미리보기 · 확장 로드맵</a>
      </div>
      <p class="hero-demo-note">AI는 판단·제안만 하고, 담당자가 승인/수정합니다. 모든 결과는 내부 운영 참고용입니다.</p>
    </section>
  `;
}

function groupExpansionView() {
  return `
    <div class="work-list">
...
function dashboardPage() {
  return `
    ...
    <section class="dashboard-grid">
      ...
      <div class="dashboard-column">
        ${panelMarkup("그룹 확장성", "은행 Hero → 계열사 확장 노드", groupExpansionView(), "expansion-panel panel-secondary")}
...
function roleDashboardPage(config) {
  return `
    ${pageHeader(config.eyebrowLabel || "담당자 유형", config.title, config.description)}
    <section class="workspace-panel rm-hero">
      <div>
...
function jbWooriCapitalDashboardContextMarkup() {
  if (typeof jbwcContextMarkup === "function") return jbwcContextMarkup();
  return `<div class="empty-state">JB우리캐피탈 전용 컨텍스트를 불러오지 못했습니다.</div>`;
}
...
function jeonsePage() {
  return `
    ${pageHeader("전세 안심 점검 · 확장 로드맵", "전세 관련 위험 신호 관리", "확장 로드맵 후보 화면입니다. Hero 실동작 범위는 여신(소상공인·SME)과 보이스피싱 2개 도메인이며, 전세 보호는 데이터 통합 이후 단계로 계획합니다.")}
  ...
  return `
    <article class="agent-card is-clickable ${agent.id === selectedAgentId && activeDetailType === "agent" ? "is-selected" : ""}" data-agent-id="${escapeHtml(agent.id)}" role="button" tabindex="0">
      <div class="item-head">
        ...
        <span>${agentExecutionBadge(agent)}<span class="status-pill ${statusClass(agent.status)}">${escapeHtml(statusLabel(agent.status))}</span></span>
      </div>
...
function orgChartView() {
  ...
  const branches = [
    ...
    {
      title: "전세 보호 라인 · 확장 로드맵",
      subtitle: "전세가율·권리관계·고객 자산노출·은행 연계 — 실동작 아님, 확장 예정 노드",
    ...
    {
      title: "사기 차단과 준법 · Hero 실동작",
      subtitle: "보이스피싱 차단, 준법 검토, 특약/계약 문구 통제",
  ...
  return `
      ...
      <div class="org-legend">
        ...
        <span class="status-pill status-approved">실동작 ${agents.filter((agent) => liveAgentIds.has(agent.id)).length}</span>
        <span class="status-pill status-new">확장 예정 ${agents.filter((agent) => !liveAgentIds.has(agent.id)).length}</span>
      </div>
    </div>
  `;
}
  ...
  return `
    <article class="org-node is-clickable ${variant} ${selected}" data-agent-id="${escapeHtml(agent.id)}" role="button" tabindex="0">
      <div class="node-topline">
        ...
        <span>${agentExecutionBadge(agent)}<span class="status-pill ${status}">${escapeHtml(statusLabel(agent.status))}</span></span>
      </div>
      <strong>${escapeHtml(agentLabel(agent))}</strong>
      <p>${escapeHtml(localizeLine(agent.role))}</p>
      <div class="node-meta">
  ...
  document.querySelectorAll("[data-affiliate]").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      if (next === "JB우리캐피탈") {
        ...
        activeDetailType = defaultDetailForView(activeView);
        const jbwcHash = typeof jbwcHashForView === "function" ? jbwcHashForView("board") : "#/jb-woori-capital";
        if (window.location.hash !== jbwcHash) {
          window.location.hash = jbwcHash;
        }
        closeRailGroups();
        render();
        notify("JB우리캐피탈 전용 화면으로 이동했습니다.");
        return;
      }
...
function applyHashRoute() {
  ...
  if (jbwcRoute) {
    ...
    activeDetailType = defaultDetailForView(activeView);
    railFilter = "JB우리캐피탈";
    if (typeof jbwcState !== "undefined") {
      jbwcState.view = jbwcRoute.view || "board";
      jbwcState.detail = jbwcRoute.caseId ? { kind: "case", id: jbwcRoute.caseId } : null;
    }
    return;
  }
